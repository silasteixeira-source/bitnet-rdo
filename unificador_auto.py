    ...
    if not df_falta_abrir.empty:
        ...
        if not permitir_abertura:
            ...
            try:
                ...
                log("Robô Selenium acionado com sucesso em segundo plano (Paralelo). A ST1 será processada imediatamente.")
            except Exception as e_pw:
                log(f"⚠️ Erro ao tentar executar abrirChamado_selenium.py: {e_pw}")
        else:
            log(f"Fora do horário comercial (Seg-Sex, 08h às 15:59). Robô bloqueado. (Agora: {agora.strftime('%A %H:%M')})")
